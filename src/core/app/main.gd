...
## Handed to any SplashScreen that has no `next_screen` of its own, so splash
## cards can chain (company -> studio -> here).
...
func _on_screen_changed(screen: Node) -> void:
	if screen is SplashScreen and (screen as SplashScreen).next_screen == null:
		(screen as SplashScreen).next_screen = first_game_screen
